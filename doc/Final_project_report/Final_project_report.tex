...
\node[inout, right=0.8cm of title] (author) {Author};
\node[inout, right=0.8cm of author] (body) {Review Body};

% BERT Embeddings Nodes
\node[proc, below=0.6cm of author] (bertTitle) {BERT Encoder};
\node[proc, right=0.8cm of bertTitle] (bertAuthor) {BERT Encoder};
\node[proc, left=0.8cm of bertTitle] (bertBody) {BERT Encoder};
\node[proc, below=0.6cm of bertBody] (bertProduct) {BERT Encoder};

% Input Features Nodes
\node[inout, below=0.6cm of bertProduct] (product) {Product Details};
\node[inout, below=0.6cm of bertAuthor] (stars) {Star Rating};

% Concatenation Nodes
\node[proc, below=2.1cm of author] (concat) {Concatenation};

% Classifier Nodes
\node[proc, below=1cm of concat] (fc) {Fully Connected};
\node[proc, below=0.5cm of fc] (relu) {ReLU};
\node[proc, below=0.5cm of relu] (dropout) {Dropout};
\node[proc, below=0.5cm of dropout] (output) {Output Layer};
